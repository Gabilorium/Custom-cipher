.PHONY: encoder decoder tests clean specific_encoder_test specific_decoder_test list_tests help

help:
	@echo "make alphabet                         - generate the cipher alphabet"
	@echo "make encoder FILE=...                 - encode a file (output saved to encoded_texts/, omit FILE for interactive mode)"
	@echo "make decoder FILE=...                 - decode a file (output saved to decoded_texts/, omit FILE for interactive mode)"
	@echo "make list_tests                       - lists all the posible tests"
	@echo "make tests                             - run the automated test suite"
...
clean:
	rm -f alphabet.json
	rm -rf __pycache__ cipher/__pycache__ alphabet_generator/__pycache__
	rm -rf .pytest_cache
	rm -rf encoded_texts decoded_texts
